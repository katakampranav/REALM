...
const Choice1Story = () => {
  ...
  // Get story from location state (from first code logic)
    ...
    const loadStory = async () => {
      ...
      gsap.to(
        { progress: 0 },
        {
          ...
          onComplete: () => {
            // Set story data from location state or fallback
            if (story) {
              setStoryData({ scenes: story })
            }
  ...
  }, [currentChapter, isLoading, showContents, storyData])

  // Navigate to specific chapter
  const navigateToChapter = (chapterIndex) => {
    setCurrentChapter(chapterIndex)
    const chapterElement = chaptersRef.current[chapterIndex]
    if (chapterElement) {
  ...
  useEffect(() => {
    ...
    const updateCurrentChapter = () => {
      ...
      chaptersRef.current.forEach((chapter, index) => {
        if (chapter) {
          ...
          const chapterTop = rect.top + scrollY
          const chapterBottom = chapterTop + rect.height

          if (scrollY + windowHeight / 2 >= chapterTop && scrollY + windowHeight / 2 <= chapterBottom) {
    ...
    return () => window.removeEventListener("scroll", updateCurrentChapter)
  }, [isLoading, storyData])
    ...
    if (isLoading) {
      ...
      chaptersRef.current.forEach((chapter) => {
        if (chapter) {
          ...
          // Add subtle pulse animation to images (from first code)
          const sceneImage = chapter.querySelector(".scene-image")
          if (sceneImage) {
            gsap.to(sceneImage, {
              scale: 1.02,
              duration: 3,
  ...
  // Handle no story case (from first code logic)
  if (!story && !isLoading) {
    return (
  ...
  const scenes = Object.entries(storyData.scenes)
  ...
  return (
    <div className="relative min-h-screen w-screen overflow-hidden bg-black">
      ...
      {/* Header with back navigation (from first code) */}
      ...
      <div className="fixed bottom-6 right-6 z-50 flex gap-3">
        <button
          onClick={() => currentChapter > 0 && navigateToChapter(currentChapter - 1)}
        ...
        <button
          onClick={() => currentChapter < totalChapters - 1 && navigateToChapter(currentChapter + 1)}
      ...
      <div ref={storyRef} className="story-container relative z-10">
        ...
        <div className="relative flex min-h-screen items-center justify-center px-6">
          <div className="border-hsla mx-auto max-w-5xl rounded-3xl bg-black/20 p-16 text-center shadow-2xl backdrop-blur-xl">
            ...
            <div className="flex items-center justify-center gap-8 font-general text-base uppercase text-blue-50/60">
              <span className="flex items-center gap-2">{totalChapters} Chapters</span>
        ...
        <div className="container mx-auto px-3 py-20 md:px-10">
          {scenes.map(([sceneKey, scene], index) => (
            <div
              key={sceneKey}
              ref={(el) => (chaptersRef.current[index] = el)}
              className={`mb-32 ${index === scenes.length - 1 ? "mb-0" : ""}`}
            >
              <div
              ...
              >
                ...
                <div className={`relative ${index % 2 === 1 ? "lg:order-2" : ""}`}>
                  ...
                  <div className="absolute -left-4 -top-4 flex size-12 items-center justify-center rounded-full border-4 border-black bg-violet-50 shadow-xl">
                    <span className="font-zentry text-lg font-black text-black">{index + 1}</span>
                  </div>
                </div>
                ...
                <div className={`space-y-8 ${index % 2 === 1 ? "lg:order-1" : ""}`}>
                  <div className="border-hsla rounded-md bg-black/40 p-10 shadow-2xl backdrop-blur-xl transition-transform duration-300 ease-out hover:scale-[1.02]">
                    ...
                    <div className="mt-8 flex items-center gap-6 border-t border-white/20 pt-6 font-general text-sm uppercase text-blue-50/50">
                      <span className="flex items-center gap-2">Chapter {index + 1}</span>
                      <span className="text-violet-300">•</span>
                      <span className="flex items-center gap-2">{scene.description.split(" ").length} words</span>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`flex size-10 items-center justify-center rounded-full font-zentry text-sm font-black ${
                          currentChapter === index ? "bg-violet-50 text-black" : "bg-white/10 text-blue-50"
                        }`}
                      >
                        {index + 1}
                      </div>
                      <div className="flex-1">
                        <h4 className="font-zentry text-lg font-black uppercase">{scene.title}</h4>
